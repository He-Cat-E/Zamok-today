/**
 * Translates all insurance.dask.* values from English into each locale JSON
 * (except en + tr). Uses google-translate-api-x (unofficial Google).
 *
 * Run from frontend: npm run i18n:translate-dask
 * Resume: npm run i18n:translate-dask -- no pl pt
 */
...
const DASK_PREFIX = "insurance.dask.";
...
async function translateLocale(fileBase, googleTo) {
  ...
  const entries = Object.entries(en)
    .filter(([k]) => k.startsWith(DASK_PREFIX))
...
async function main() {
  ...
  if (targets.length === 0) {
    console.error("No locales. Usage: node scripts/translate-dask-i18n.mjs [de fr ...]");
    process.exit(1);
  }
