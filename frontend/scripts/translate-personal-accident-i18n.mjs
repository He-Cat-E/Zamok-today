/**
 * Translates all insurance.personalAccident.* values from English into each
 * locale JSON (except en + tr). Uses google-translate-api-x (unofficial Google).
 *
 * Run from frontend: npm run i18n:personal-accident
 * Resume one language: npm run i18n:personal-accident -- no
 * Multiple: npm run i18n:personal-accident -- no pl pt
 */
...
const PA_PREFIX = "insurance.personalAccident.";

/** Locale filename (no .json) → Google Translate `to` code */
...
async function translateBatchWithRetry(strings, googleTo) {
  ...
  /** Fallback: single-string translate */
  const out = [];
...
async function translateLocale(fileBase, googleTo) {
  ...
  const entries = Object.entries(en)
    .filter(([k]) => k.startsWith(PA_PREFIX))
...
async function main() {
  ...
  if (targets.length === 0) {
    console.error("No locales. Usage: node scripts/translate-personal-accident-i18n.mjs [de fr ...]");
    process.exit(1);
  }
