...
type DestinationTicketCardProps = {
  ...
  fromCity?: string;
  /** Currency for price formatting (e.g. USD, EUR, TRY) */
  currencyCode?: string;
...
export function DestinationTicketCard({
  ...
  const normalizedCurrency = String(ticket.currencyCode || currencyCode || "USD")
    .trim()
    .toUpperCase();
  ...
  const formattedPrice = (() => {
    try {
      ...
      return `${normalizedCurrency} ${Math.round(ticket.price)}`;
    }
  })();
